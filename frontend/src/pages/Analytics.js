    ...
    const printContent = `
      ...
      <head>
        ...
        <style>
          ...
          h2 { margin-top: 30px; font-size: 16px; color: #666; }
          .kpi-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin: 20px 0; }
          .kpi { padding: 20px; background: #f3f4f6; border-radius: 8px; text-align: center; }
          .kpi-value { font-size: 32px; font-weight: bold; color: #111; }
      ...
      <body>
        ...
        <div class="kpi-grid">
          <div class="kpi">
            <div class="kpi-value">${data?.total_visits || 0}</div>
            <div class="kpi-label">Total Visits</div>
          </div>
          <div class="kpi">
            <div class="kpi-value">${data?.unique_patients || 0}</div>
            <div class="kpi-label">Unique Patients</div>
          </div>
          <div class="kpi">
            <div class="kpi-value">${data?.new_patients || 0}</div>
            <div class="kpi-label">New Patients</div>
          </div>
          <div class="kpi">
            <div class="kpi-value">${data?.avg_visits_per_day || 0}</div>
        ...
        <table>
          <thead><tr><th>Consultant</th><th>Visits</th></tr></thead>
          <tbody>
            ${consultantData.map(c => `<tr><td>${c.name}</td><td>${c.count}</td></tr>`).join('')}
          </tbody>
        </table>
        ...
        <table>
          ...
          <tbody>
            ${dailyData.map(d => `<tr><td>${d.date}</td><td>${d.count}</td></tr>`).join('')}
          </tbody>
        </table>
        
        <div class="footer">Generated by Just Vitality Clinic System on ${new Date().toLocaleString()}</div>
      </body>
      </html>
    `;
